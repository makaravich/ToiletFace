										...
										if ( isset( $item['variation_id'] ) && 0 !== $item['variation_id'] ) {
											...
											foreach ( $item['item_meta'] as $key => $value ) {
												if ( ! ( 0 === strpos( $key, '_' ) ) ) {
													...
													if ( isset( $term_wp->name ) ) {
														echo '<br><p>' . wp_kses_post( $attribute_name . ':' . $term_wp->name );
														echo '</p>';
													} else {
														echo '<br><p>' . wp_kses_post( $attribute_name . ':' . $value );
														echo '</p>';
													}
												}
											}
											...
											foreach ( $item['item_meta'] as $key => $value ) {
												if ( ! ( 0 === strpos( $key, '_' ) ) ) {
													...
													// BEGIN Upwork 
													if (in_array($key, ['Preview', 'Upload your image'])) {
														$link = $value;
														try {
															$link = new \SimpleXMLElement($link);
															$link = $link['href'];
															$value = '<img src="' . $link . '" style="max-width: 250px;">';
														} catch (Exception $ex) {
															// do nothing
														}
													}
													...
													echo '<br><p>' . wp_kses_post( $key . ':' . $value );
													echo '</p>';
												}
											}
										}
